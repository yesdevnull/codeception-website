...
> Use actor classes to set common actions which can be used across a suite.

### Attributes in Helpers

You can annotate actor/helper methods and parameters with PHP attributes and have them reflected in the generated class files.

```php
// tests/support/UnitHelper.php
...
class CliHelper extends \Codeception\Module
{
    ...
    public function getArrayOfValues(): array
    {
        return [
            'input' => new stdClass(),
            'date' => new DateTime(),
...
The previous example would generate the following:
...
trait CliGuyActions
{
     ...
     *
     *
     * @see \Codeception\Module\CliHelper::getArrayOfValues()
     */
    #[\JetBrains\PhpStorm\ArrayShape(["input" => "stdClass", "date" => "DateTimeInterface"])]
    public function getArrayOfValues(): array {
        return $this->getScenario()->runStep(new \Codeception\Step\Action('getArrayOfValues', func_get_args()));
    }
```

IDEs like PhpStorm will correctly infer the types for classes even when they are within a string.

> Note that named parameters in attribute arguments are not supported at this time.
